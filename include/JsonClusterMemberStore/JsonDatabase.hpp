 ...
 * @file Database.hpp
 *
 * This file specifies the JSON implementation of an abstract interface for C++
 * components to access a database in an exclusive manner in the context of a
 * member of a larger cluster (such as in the Raft Consensus Algorithm).  In
...
namespace ClusterMemberStore {
    ...
    class JsonDatabase : public Database {
        // Lifecycle
    public:
        ~JsonDatabase() noexcept;
        JsonDatabase(const JsonDatabase&) = delete;
        JsonDatabase(JsonDatabase&&) noexcept;
        JsonDatabase& operator=(const JsonDatabase&) = delete;
        JsonDatabase& operator=(JsonDatabase&&) noexcept;

        // Methods
    public:
        JsonDatabase();
        bool Open(std::shared_ptr< SystemAbstractions::IFileSystemEntry > store);
        bool Commit();
